...
public class SalesController {
	...
	public String sales(Model model) {
		...
		for(SalesModel sale:salesList) {
			...
			salesDisplayTable.setCustomerName(sale.getCustomer().getFullName());
			salesDisplayTable.setProductName(sale.getProduct().getProductName());
	...
	public String saveSales(@Valid @ModelAttribute("salesDisplay")SalesDisplay salesDisplay, BindingResult bindingResult) {
		...
		salesService.saveSalesDisplayToSales(salesDisplay.getQuantities(),productService.getProductById(salesDisplay.getProductId()),customerService.getCustomerById(salesDisplay.getCustomerId()));		
		List<SalesModel> salesList= salesService.getAllSales();
